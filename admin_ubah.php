<?php session_start();
	if(session_is_registered('username')){
		...
		$query = mysql_query("select * from tabel_loginadmin where id_user='$id_user'") or die(mysql_error());
	...
	if(isset($_POST['submit'])){
		...
		if (empty($username) && empty($password)) {
			header('location:admin_ubah.php?error=1');
		} else if (empty($username)) {
			header('location:admin_ubah.php?error=2');
		} else if (empty($password)) {
			header('location:admin_ubah.php?error=3');
		} else {
			mysql_query("UPDATE tabel_loginadmin SET username='$username',password='$password',nama='$nama' where id_user='$id_user'") or die (mysql_error());
...
<title>Joe Jersey's - Sunting Admin</title>
					...
					<form method="post" action="" >
						<table cellpadding="2" cellspacing="2" border="0" width="210">
							<tr>
								...
								<td width="10">:</td>
								<td width="110"><input type="text" name="username" value="<?php echo $data['username']; ?>"></td>
							</tr>
							<tr>
								<td width="90">Password</td>
								<td width="10">:</td>
								<td width="110"><input type="password" name="password" value="<?php echo $data['password']; ?>"></td>
							</tr>
							<tr>
								<td width="90">Nama</td>
								<td width="10">:</td>
								<td width="110"><input type="text" name="nama" value="<?php echo $data['nama']; ?>"></td>
							</tr>
							<tr>
								<td colspan="3">
									<?php
